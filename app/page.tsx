...
const getEduMediaUrl = (e: EducationPost) =>
  e.mediaUrl || e.media_url || FALLBACK_IMG;
const isPengumuman = (cat?: string) =>
  ["pengumuman", "PENGUMUMAN", "Pengumuman"].includes(cat || "");
...
const navHref = (item: string) =>
  ["berita", "edukasi", "galeri"].includes(item.toLowerCase())
    ? `/${item.toLowerCase()}`
    : `#${item.toLowerCase()}`;
...
const Navbar = () => {
  ...
  return (
    <>
      <nav
        className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? "bg-slate-900/95 backdrop-blur-md border-b border-slate-800 py-3 shadow-md" : "bg-transparent py-5"}`}
      >
        <div className="max-w-7xl mx-auto px-6 lg:px-8 flex items-center justify-between">
          ...
          >
            ...
            <div className="flex flex-col">
              <h1 className="font-bold text-sm tracking-tight transition-colors text-white">
          ...
          <div className="hidden md:flex items-center gap-2">
            {NAV_LINKS.map((item) => (
              <Link
                key={item}
                href={navHref(item)}
                className="px-4 py-2 rounded-lg text-sm font-semibold transition-all focus-visible:ring-2 focus-visible:ring-emerald-500 text-white/90 hover:text-white hover:bg-white/10"
              >
                {item}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <Link
              href="/login"
              className="hidden md:inline-flex text-sm font-semibold px-4 py-2 rounded-lg transition-colors focus-visible:ring-2 focus-visible:ring-emerald-500 text-white hover:bg-white/10"
            >
              Login
            </Link>
            ...
            <Link
              href="/Warga"
              className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold px-4 py-2 md:px-5 md:py-2.5 rounded-lg transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-emerald-500"
            >
              <Leaf size={16} /> Lapor
            </Link>
            ...
            <button
              aria-label="Menu"
              className="md:hidden p-2 rounded-lg transition-colors text-white bg-white/10 backdrop-blur-sm hover:bg-white/20"
            ...
            <div className="p-6 flex flex-col gap-4">
              {NAV_LINKS.map((item) => (
                <Link
                  ...
                  href={navHref(item)}
                  onClick={() => setMobileMenuOpen(false)}
                  className="text-lg font-bold text-slate-700 flex justify-between items-center border-b border-slate-50 pb-4 hover:text-emerald-600 transition-colors"
                >
...
const HeroSection = () => (
  <header className="relative w-full min-h-[85vh] flex items-center justify-center overflow-hidden">
    <div className="absolute inset-0 z-0">
      <Image
        src="https://www.chubb.com/content/dam/chubb-sites/chubb/global/images/things/green-forest-trees-1280x528.jpg/jcr:content/renditions/cq5dam.web.1280.1280.jpeg"
        alt="Background Toba"
        fill
    ...
    <div className="max-w-4xl mx-auto px-6 relative z-10 w-full flex flex-col items-start justify-start text-left mt-16">
      ...
      >
        ...
        >
          Menjaga{" "}
          <span className="text-emerald-400">Kelestarian Lingkungan</span> Toba
        </motion.h1>
        <motion.p
          variants={fadeUp}
          className="text-lg text-slate-200 mb-10 leading-relaxed max-w-2xl"
        >
          Sinergi pemerintah dan masyarakat dalam mewujudkan lingkungan yang
          asri, bersih, dan berkelanjutan untuk generasi mendatang.
        </motion.p>
      </motion.div>
    </div>
  </header>
  ...
  <section id="tentang" className="py-24 px-6 lg:px-8 max-w-7xl mx-auto">
    <div className="text-center mb-16">
      ...
      <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 tracking-tight">
        Toba <span className="text-emerald-600">Mantap</span> 2029
      </h2>
      <p className="text-slate-500 mt-4 max-w-2xl mx-auto">
        Visi dan misi pembangunan Kabupaten Toba menuju daerah maju, sejahtera,
        dan berkelanjutan.
      </p>
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      >
        ...
        <h3 className="text-emerald-900 text-xl font-bold mb-3">
          Tentang DLH Toba
        </h3>
        <p className="text-emerald-800 text-sm leading-relaxed flex-1">
          Dinas Lingkungan Hidup Kabupaten Toba berkomitmen mewujudkan tata
          kelola lingkungan yang berkelanjutan, melindungi ekosistem Danau Toba,
      ...
      >
        ...
        <p className="text-slate-700 text-base font-semibold leading-relaxed flex-1">
          "Maju Daerahnya, Sejahtera Rakyatnya dan Berkelanjutan
          Pembangunannya."
        </p>
      </motion.div>
...
export default function HomePage() {
  ...
  const beritaPosts = posts
    .filter((p) => !isPengumuman(p.category))
    .slice(0, 3);
  const pengumumanPosts = posts
    .filter((p) => isPengumuman(p.category))
    .slice(0, 4);
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      if (e.key === "Escape")
        lightbox ? setLightbox(null) : setSelectedAlbum(null);
      if (lightbox) {
      ...
      <section id="berita" className="py-24 bg-white border-y border-slate-200">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <p className="text-slate-500 mt-4 max-w-2xl mx-auto">
              Informasi terkini seputar kegiatan dan kebijakan Dinas Lingkungan
              Hidup Kabupaten Toba.
            </p>
          </div>
                ...
                <div className="flex flex-col gap-4 flex-1">
                  {beritaPosts.map((post) => (
                    <Link
                      ...
                      key={post.id}
                      className="group flex flex-col sm:flex-row gap-5 p-4 rounded-2xl hover:bg-slate-50 border border-transparent hover:border-slate-200 transition-colors focus-visible:ring-2 focus-visible:ring-emerald-500"
                    >
                      ...
                      <div className="flex flex-col justify-center flex-1 min-w-0">
                        <span className="text-emerald-600 text-[10px] font-bold uppercase tracking-wider mb-1 block">
                          {post.category || "Berita"}
                        </span>
                        <h4 className="font-bold text-slate-900 group-hover:text-emerald-600 transition-colors mb-2 line-clamp-2 leading-tight">
                          {post.title}
                        </h4>
                        <div className="flex items-center gap-2 text-xs text-slate-500 font-medium mt-auto">
                          <Clock size={12} />{" "}
                          {fmtDate(post.createdAt || post.date)}
                        </div>
                      </div>
                    </Link>
                  ))}
                  <Link
                    href="/berita"
                    className="mt-4 inline-flex items-center gap-2 text-emerald-600 font-semibold hover:text-emerald-700 transition-colors w-max text-sm px-4"
                  >
                ...
                <div className="flex flex-col gap-3 flex-1">
                  {pengumumanPosts.map((post) => (
                    <Link
                      href={`/berita/${post.slug || post.id}`}
                      key={post.id}
                      className="group p-5 rounded-2xl bg-slate-50 border border-slate-100 hover:border-amber-200 transition-colors flex gap-4 focus-visible:ring-2 focus-visible:ring-amber-500 items-center"
                    >
                      <div className="w-10 h-10 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center shrink-0">
                        <Leaf size={18} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="font-bold text-slate-900 group-hover:text-amber-700 transition-colors mb-1 line-clamp-2 leading-tight text-sm">
                          {post.title}
                        </h4>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
            <div>
              ...
              <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 tracking-tight">
                Edukasi <span className="text-emerald-600">Lingkungan</span>
              </h2>
              <p className="text-slate-500 mt-2 max-w-2xl">
                Materi edukasi untuk meningkatkan kesadaran dan kepedulian
                terhadap lingkungan.
              </p>
            </div>
            <Link
              href="/edukasi"
              className="hidden md:inline-flex items-center gap-2 px-5 py-2.5 bg-white border border-slate-200 hover:border-emerald-300 hover:text-emerald-700 rounded-lg text-slate-700 font-semibold transition-colors focus-visible:ring-2 focus-visible:ring-emerald-500 text-sm"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {loading ? (
              [1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-[350px] bg-slate-200 animate-pulse rounded-2xl"
                />
              ))
            ) : educations.length > 0 ? (
              educations.map((edu, i) => (
                ...
                >
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    <h3 className="font-bold text-lg text-slate-900 mb-3 line-clamp-2 group-hover:text-emerald-600 transition-colors leading-snug">
                      {getEduTitle(edu)}
                    </h3>
                    <p className="text-slate-500 text-sm line-clamp-3 mb-6 flex-1 leading-relaxed">
                      {stripHtml(getEduDesc(edu))}
                    </p>
                    <Link
                      href={`/edukasi/${edu.id}`}
                      className="inline-flex items-center gap-2 text-emerald-600 font-semibold text-sm transition-colors mt-auto hover:text-emerald-700 focus-visible:outline-none"
                    >
              ...
              <p className="text-slate-500 italic col-span-3 text-center py-12">
          ...
          <div className="mt-8 md:hidden text-center">
            <Link
              href="/edukasi"
              className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-white border border-slate-200 hover:border-emerald-300 rounded-lg text-slate-700 font-semibold w-full sm:w-auto"
            >
              Lihat Semua Edukasi <ArrowRight size={16} />
            </Link>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
            <div>
              ...
              <p className="text-slate-500 mt-2 max-w-2xl">
                Dokumentasi kegiatan dan program Dinas Lingkungan Hidup
                Kabupaten Toba.
              </p>
            </div>
            <Link
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {loading ? (
              [1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-[350px] bg-slate-100 animate-pulse rounded-2xl"
                />
              ))
              ...
              albums.slice(0, 3).map((album, i) => (
                ...
                >
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    <h3 className="font-bold text-lg text-slate-900 mb-2 line-clamp-2 group-hover:text-emerald-600 transition-colors leading-snug">
                      {album.title}
                    </h3>
                    {album.description && (
      ...
      <footer className="bg-slate-950 text-slate-300 pt-20 pb-10 border-t border-slate-800">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
            <div className="lg:col-span-1">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-emerald-500 text-[10px] font-semibold tracking-widest uppercase">
                    Kabupaten Toba
                  </p>
                </div>
              </div>
              <p className="text-slate-400 text-sm leading-relaxed mb-6">
                Mewujudkan lingkungan hidup yang asri, bersih, dan berkelanjutan
                untuk masyarakat Toba yang sejahtera.
              </p>
              <div className="flex gap-3">
                {[
                  {
                    Icon: Facebook,
                    href: "https://www.facebook.com/share/1DyWuusTwD/?mibextid=wwXIfr",
                  },
                  {
                    Icon: Instagram,
                    href: "https://www.instagram.com/dlh_kab.toba?igsh=aGI5NjhyYW5zOXQ4",
                  },
            ...
            <div>
              <h4 className="text-white font-semibold mb-6 text-sm tracking-wider">
                Navigasi Utama
              </h4>
                ...
                {[
                  {
                    ...
                    path: "https://dislindup.tobakab.go.id/tugas-pokok-dan-fungsi/",
                  },
                  {
                    name: "Dokumen RPJMD",
                    path: "https://dislindup.tobakab.go.id/rpjmd/",
                  },
                  {
                    name: "Dokumen RENSTRA",
                    path: "https://dislindup.tobakab.go.id/renstra/",
                  },
                  {
                    name: "Struktur Organisasi",
                    path: "https://dislindup.tobakab.go.id/struktur-organisasi/",
                  },
                ].map((item) => (
                  <li key={item.name}>
                    <a
                ...
                <ul className="space-y-4 text-sm">
                  <li className="flex items-start gap-3">
                    ...
                    <span className="text-slate-400">
                      Jl. Hutabulu Mejan No. 14, Sibola Hotangsas, Balige, Toba,
                      Sumatera Utara
                    </span>
                  </li>
                  <li className="flex items-center gap-3">
                    <Mail size={18} className="text-slate-500 shrink-0" />{" "}
                    <span className="text-slate-400">dlhtoba@gmail.com</span>
                  </li>
                </ul>
              </div>
            </div>
          ...
          <div className="border-t border-slate-800 pt-8 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="flex gap-6 text-sm font-medium">
              <Link
                href="#"
                className="text-slate-500 hover:text-emerald-400 transition-colors focus-visible:outline-none"
              >
                Kebijakan Privasi
              </Link>
              <Link
          ...
          >
            <div className="sticky top-0 z-[110] bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between shadow-sm">
              <button
                ...
                className="flex items-center gap-2 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 px-4 py-2 rounded-lg font-semibold text-sm transition-colors focus-visible:ring-2 focus-visible:ring-emerald-500"
              >
              ...
              <div className="sm:hidden">
                <h2 className="font-bold text-slate-900 text-sm truncate max-w-[140px]">
                  {selectedAlbum.title}
                </h2>
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {selectedAlbum.photos?.map((photo, idx) => (
                  <div
                    key={photo.id}
                    onClick={() =>
                      setLightbox({ photos: selectedAlbum.photos!, index: idx })
                    }
                    className="aspect-square rounded-xl overflow-hidden relative group cursor-pointer border border-slate-200 bg-slate-100 focus-visible:ring-2 focus-visible:ring-emerald-500"
                    tabIndex={0}
                    role="button"
                  >
                    <Image
        ...
        {lightbox && (
          ...
          >
            <button
              ...
              className="absolute top-6 right-6 w-10 h-10 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition-colors focus-visible:ring-2 focus-visible:ring-white z-10"
            >
              <X size={20} />
            </button>
            {lightbox.photos.length > 1 && (
              <>
                <button
                  ...
                  className="absolute left-4 md:left-10 top-1/2 -translate-y-1/2 w-10 h-10 md:w-14 md:h-14 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition-colors focus-visible:ring-2 focus-visible:ring-white z-10"
                >
                  <ChevronLeft size={24} />
                </button>
                <button
                  ...
                  className="absolute right-4 md:right-10 top-1/2 -translate-y-1/2 w-10 h-10 md:w-14 md:h-14 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition-colors focus-visible:ring-2 focus-visible:ring-white z-10"
                >
            ...
            <div className="flex flex-col items-center max-w-5xl w-full">
              <div className="relative w-full max-h-[80vh] flex justify-center">
                <img
                  ...
                  alt={lightbox.photos[lightbox.index].caption || "Foto Galeri"}
                  onError={(e) => {
              ...
              <div className="mt-4 text-center text-white">
                ...
                <p className="text-slate-400 font-semibold text-xs tracking-widest">
