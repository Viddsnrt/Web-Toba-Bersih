...
const findHeaderRowIndex = (aoa: any[][]): number => {
  for (let i = 0; i < Math.min(aoa.length, 15); i++) {
    ...
    aoa[i].forEach((c) => {
      ...
      if (!cell) return;
      if (cell === "no" || cell === "no.") matches++;
      else if (cell.includes("nama") || cell.includes("pelanggan")) matches++;
      else if (cell.includes("alamat")) matches++;
      else if (cell.includes("jenis")) matches++;
    });
...
// ✅ BARU: ambil nilai kolom berdasarkan kata kunci, bukan nama kolom persis
const getField = (row: any, keywords: string[]): string => {
...
const parseExcel = (file: File) => {
  ...
  reader.onerror = () => {
    alert("Gagal membaca file. Coba file lain atau simpan ulang sebagai .xlsx.");
  };
  ...
  reader.onload = (e) => {
    try {
      ...
      // ✅ Loop semua sheet, DAN cari header asli per sheet (bukan asumsi baris 1)
      const allRows: any[] = [];
      wb.SheetNames.forEach((sheetName) => {
        ...
        const json: any[] = XLSX.utils.sheet_to_json(ws, {
          defval: "",
          range: headerRowIdx, // ✅ mulai baca dari baris header yang benar
        });
        allRows.push(...json);
      });

      // ✅ Log header asli ke console untuk debugging
      if (allRows.length > 0) {
        console.log("Header terbaca dari file:", Object.keys(allRows[0]));
      }

      const mapped: ImportRow[] = allRows
        .map((r) => ({
          nama: getField(r, ["nama pelanggan", "nama"]),
          alamat: getField(r, ["alamat"]),
          jenisUsaha:
            getField(r, ["jenis usaha", "jenis retribusi"]) || "Rumah Tangga",
          status: "pending" as const,
        }))
        // ✅ buang baris kosong & sisa baris "nomor urut" (nama = "1", "2", dst)
        .filter((r) => r.nama && !/^\d+$/.test(r.nama));

      if (mapped.length === 0) {
        const headerInfo =
          allRows.length > 0
            ? `Header yang terbaca dari file Anda:\n${Object.keys(allRows[0]).join(", ")}`
            : "File tidak berisi data sama sekali.";
        alert(
          `Tidak ada baris valid yang terbaca dari file ini.\n\n${headerInfo}\n\nPastikan ada kolom "Nama Pelanggan" (atau "Nama").`,
        );
        return;
      }
      ...
      alert(`Gagal membaca isi file Excel: ${err.message}`);
    }
  };
...
const handleFile = (file: File) => {
  if (!file.name.match(/\.(xlsx|xls)$/i)) {
    // ✅ TAMBAHAN: kasih tahu user kenapa file ditolak, jangan diam-diam return
    alert(`File "${file.name}" bukan format .xlsx/.xls. Pilih file Excel yang valid.`);
    return;
  }
  setDone(false);
  setSummary(null);
...
const handleImport = async () => {
  const validated = rows.map((r) =>
    !r.nama
      ? { ...r, status: "error" as const, errorMsg: "Nama wajib diisi" }
      : r,
  );
  setRows(validated);
  ...
  try {
    ...
    const res = await axios.post(
      ...
      {
        pelanggan: valid.map(({ nama, alamat, jenisUsaha }) => ({
          nama,
          alamat,
          jenisUsaha,
        })),
        locationId: selectedLocation || null, // ✅ opsional, boleh null
      },
            ...
            ? {
                ...
                status:
                  r.status === "success"
                    ? ("success" as const)
                    : ("error" as const),
                errorMsg: r.status === "error" ? r.message : undefined,
              }
            : {
                ...row,
                status: "error" as const,
                errorMsg: "Tidak ada respons server",
              };
        }),
      );
  ...
  alert(
    err?.response?.data?.message ||
      "Gagal mengirim data ke server. Coba lagi atau periksa koneksi Anda.",
  );
} finally {
  setImporting(false);
  setDone(true);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4">
      ...
      >
        <div className="px-8 py-6 border-b border-gray-100 flex justify-between items-start bg-gray-50/50">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              Format sesuai data retribusi persampahan · Maks 500 baris
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          {rows.length === 0 && (
            ...
            >
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Format .xlsx atau .xls · Maks 500 baris
                </p>
              </div>
              <input
                ref={fileRef}
          ...
          {rows.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-bold text-gray-800">
                  📄 {fileName} —{" "}
                  <span className="text-[#4A6D55]">{rows.length} baris</span>
                  {done && (
        ...
        <div className="border-t border-gray-100 px-8 py-5 flex items-center justify-between gap-3 bg-gray-50/50">
          <button
            onClick={onClose}
            className="px-6 py-3 rounded-2xl text-gray-600 font-bold hover:bg-gray-100 transition-all text-sm"
          >
            {done ? "Tutup" : "Batal"}
          </button>
          <div className="flex gap-3">
           {rows.length > 0 && !done && (
            ...
            >
                {importing ? (
                  <>
                    <Loader2 size={15} className="animate-spin" /> Mengimport...
                  </>
                ) : (
                  <>
                    <Upload size={15} /> Import {rows.length} Pelanggan
                  </>
                )}
              </button>
            )}
...
export default function ManagePelanggan() {
  ...
  const [selectedPelangganForDelete, setSelectedPelangganForDelete] =
  ...
  const fetchList = async (page = 1) => {
    ...
    try {
      ...
      showAlert(
        "error",
        "Gagal memuat data",
        "Data pelanggan tidak bisa dimuat.",
  ...
  // ✅ Fetch locations dari endpoint wilayah yang benar (samakan dengan ManajemenRute)
  const fetchLocations = async () => {
    const token = localStorage.getItem("token");
    if (!token) return;
    try {
      const res = await axios.get(`${API_BASE}/wilayah`, getAuthConfig());
      // Tangani berbagai bentuk response: array langsung, atau { data: [...] }, atau { success, data }
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.jenisUsaha.trim())
      errors.jenisUsaha = "Jenis usaha wajib dipilih";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingItem && originalData) {
      ...
      if (!hasChanged) {
        ...
        showAlert(
          "info",
          "Tidak Ada Perubahan",
          "Data pelanggan tidak mengalami perubahan apapun.",
        );
        return;
      }
    }
    ...
    try {
      ...
      if (editingItem) {
        await axios.put(
          `${API_BASE}/pelanggan/${editingItem.id}`,
          payload,
          getAuthConfig(),
        );
      } else {
  ...
  const handleExportAll = async () => {
    ...
    try {
      const res = await axios.get(`${API_BASE}/pelanggan/export`, {
        ...getAuthConfig(),
        responseType: "blob",
      });
      triggerDownload(
        res.data,
        `pelanggan_retribusi_${new Date().toISOString().split("T")[0]}.xlsx`,
      );
      showAlert(
        "success",
        "Export berhasil",
        "Data pelanggan berhasil diunduh.",
      );
    } catch (error: any) {
      showAlert(
        "error",
  ...
  const handleExportByLocation = async () => {
    if (!filterLocation) {
      showAlert(
        "error",
        "Pilih wilayah",
        "Pilih wilayah terlebih dahulu untuk export.",
      );
      return;
    }
    ...
    try {
      ...
      const locName =
        locations.find((l) => l.id === filterLocation)?.name ?? "lokasi";
      triggerDownload(
        res.data,
        `pelanggan_${locName.replace(/\s+/g, "_")}_${new Date().toISOString().split("T")[0]}.xlsx`,
      );
      showAlert(
        "success",
        "Export berhasil",
        `Data wilayah ${locName} berhasil diunduh.`,
      );
    } catch (error: any) {
      showAlert(
        "error",
        "Gagal export",
        "Data wilayah gagal diunduh.",
        getErrorMessage(error, "Terjadi kesalahan."),
      );
    } finally {
      setExportingLoc(false);
    }
  };
    ...
    <div className="w-full space-y-6 md:space-y-8 p-4 md:p-6 text-black">
      ...
      <div className="bg-gradient-to-r from-[#DDE9E1] to-[#E8F1EB] rounded-[24px] p-8 shadow-sm border border-white/50 relative overflow-hidden">
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-3xl font-extrabold text-[#1A2E35] tracking-tight uppercase">
            Manajemen Pelanggan Retribusi
          </h1>
          <p className="text-[#5B7078] mt-2 font-medium">
            Kelola daftar pelanggan retribusi persampahan per wilayah
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">
        {[
          ...
          <div
            key={i}
            className="bg-white p-4 md:p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4 hover:shadow-md transition-all"
          >
          ...
          <ImportModal
            ...
            onDone={() => {
              ...
              showAlert(
                "success",
                "Import selesai",
                "Data pelanggan berhasil diimport ke sistem.",
              );
            }}
          />
        )}
              ...
              <tbody className="divide-y divide-gray-50">
                {list.length === 0 ? (
                  <tr>
                    <td
                      colSpan={6}
                      className="px-6 py-16 text-center text-sm text-gray-400 italic"
                    >
                      {searchTerm
                      ...
                      <td className="px-6 py-5">
                        <div className="flex gap-1.5">
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              setSelectedPelangganForDelete(p);
              ...
              <div className="p-6 space-y-5">
                ...
                <div className="bg-gray-50 p-5 rounded-2xl border border-gray-100 space-y-4">
                  {[
                    ...
                    {
                      label: "Jenis Usaha",
                      value: viewingItem.jenisUsaha || "—",
                    },
                    {
                      label: "Wilayah",
                      value: viewingItem.location?.name || "Belum ditetapkan",
                    }, // ✅ ganti dari Supir
                  ].map(({ label, value }) => (
                    <div key={label}>
            ...
            >
              <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/70">
                <h3 className="font-extrabold text-gray-800 flex items-center gap-2.5 tracking-tight">
                  <div
                    className={`p-1.5 rounded-lg ${editingItem ? "bg-amber-50 text-amber-600" : "bg-green-50 text-[#4A6D55]"}`}
                  >
                    <User size={18} />
                  </div>
                  {editingItem
              ...
              <form
                key={
                  editingItem ? `edit-${editingItem.id}` : "create-pelanggan"
                }
              ...
              >
                {Object.keys(formErrors).length > 0 && (
                  <div className="p-3 rounded-xl bg-red-50 border border-red-100 text-red-600 text-xs font-semibold flex items-start gap-2">
                    ...
                    <span>
                      Lengkapi semua data yang wajib diisi sebelum menyimpan.
                    </span>
                  </div>
                )}
                ...
                <div className="flex flex-col space-y-1.5">
                  ...
                  <div className="relative">
                    <User
                      size={16}
                      className={`absolute left-4 top-1/2 -translate-y-1/2 ${formErrors.nama ? "text-red-400" : "text-gray-400"}`}
                    />
                    <input
                      type="text"
                      value={formData.nama}
                ...
                <div className="flex flex-col space-y-1.5">
                  ...
                  <div className="relative">
                    <MapPin
                      size={16}
                      className={`absolute left-4 top-1/2 -translate-y-1/2 ${formErrors.alamat ? "text-red-400" : "text-gray-400"}`}
                    />
                    <input
                      type="text"
                ...
                <div className="flex flex-col space-y-1.5">
                  ...
                  <div className="relative">
                    <MapPin
                      size={16}
                      className={`absolute left-4 top-1/2 -translate-y-1/2 ${formErrors.locationId ? "text-red-400" : "text-gray-400"}`}
                    />
                    <select
                  ...
                  {locations.length === 0 && (
                    <p className="text-xs text-amber-500 ml-1 flex items-center gap-1">
                      <AlertCircle size={11} /> Belum ada wilayah terdaftar.
                      Hubungi admin.
                    </p>
                  )}
                </div>
