  ...
  return (
    <div ref={wrapperRef} className="relative">
      ...
      <input
        ...
        autoComplete="off"
        className={`w-full pl-11 pr-10 py-3 bg-gray-50/80 rounded-xl outline-none text-sm sm:text-base font-medium transition-all border ${
          error
...
function ImportModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-3 sm:p-4">
      <motion.div
      ...
      >
        <div className="px-4 sm:px-8 py-4 sm:py-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-gray-50/50">
          <div>
            <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Import Data</span>
            <h3 className="text-lg sm:text-xl font-extrabold text-gray-900 flex items-center gap-2 mt-1 tracking-tight">
          ...
          <button onClick={onClose} className="p-2 hover:bg-white rounded-full transition-colors text-gray-400 self-end sm:self-auto">
            <X size={20} />
          </button>
        </div>

        <div className="overflow-y-auto flex-1 p-4 sm:p-8 space-y-5">
          {/* Template */}
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 rounded-2xl bg-gray-50 border border-gray-100 px-4 sm:px-5 py-4">
            <div>
            ...
            <button onClick={downloadTemplate}
              className="flex items-center gap-2 rounded-xl bg-white border border-gray-200 px-4 py-2 text-sm font-medium text-[#4A6D55] shadow-sm hover:bg-gray-50 transition w-full sm:w-auto justify-center">
              <FileSpreadsheet size={15} /> Unduh Template
            </button>
          </div>

          {/* Pilih Wilayah/Lokasi */}
          <div className="rounded-2xl border border-blue-200 bg-blue-50 px-4 sm:px-5 py-4 space-y-2">
            ...
            <select
              ...
              className="w-full rounded-xl border border-blue-300 bg-white px-4 py-2.5 text-sm text-gray-700 outline-none focus:border-[#4A6D55]"
            >
          ...
          {/* Summary */}
          {done && summary && (
            <div className="rounded-2xl border border-gray-100 bg-white px-4 sm:px-5 py-4 flex flex-wrap items-center gap-3 sm:gap-4 shadow-sm">
          ...
          {rows.length === 0 && (
            <div
              ...
              className={`flex flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed px-6 py-10 sm:py-14 cursor-pointer transition
                ${dragOver ? "border-[#4A6D55] bg-green-50" : "border-gray-200 bg-gray-50 hover:border-[#4A6D55] hover:bg-green-50/40"}`}
            >
              <div className="h-14 w-14 sm:h-16 sm:w-16 flex items-center justify-center rounded-2xl bg-green-100 text-[#4A6D55]">
                {/* ✅ Perbaikan: tidak pakai sm:size, pakai className */}
                  ...
                  <thead className="bg-gray-50">
                    <tr>
                      {["#", "Nama Pelanggan", "Alamat", "Jenis Usaha", "Status"].map((h) => (
                        <th key={h} className="px-3 sm:px-4 py-3 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50 bg-white">
                    {rows.map((row, i) => (
                      <tr key={i} className={row.status === "error" ? "bg-red-50" : row.status === "success" ? "bg-green-50" : ""}>
                        ...
                        <td className="px-3 sm:px-4 py-3 font-bold text-gray-900 text-sm">
                          {row.nama || <span className="text-red-400 italic text-xs">kosong</span>}
                        </td>
                        <td className="px-3 sm:px-4 py-3 text-gray-500 text-sm">{row.alamat || "-"}</td>
                        <td className="px-3 sm:px-4 py-3 text-gray-500 text-sm">{row.jenisUsaha || "-"}</td>
        ...
        <div className="border-t border-gray-100 px-4 sm:px-8 py-4 sm:py-5 flex flex-col sm:flex-row items-center justify-between gap-3 bg-gray-50/50">
          <button onClick={onClose} className="w-full sm:w-auto px-6 py-3 rounded-2xl text-gray-600 font-bold hover:bg-gray-100 transition-all text-sm">
            {done ? "Tutup" : "Batal"}
          </button>
          <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            {rows.length > 0 && !done && (
              <button onClick={handleImport} disabled={importing || !selectedLocation}
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 rounded-2xl bg-[#4A6D55] text-white text-sm font-bold hover:bg-[#395542] transition-all shadow-lg shadow-green-900/20 disabled:opacity-50 active:scale-95">
            ...
            {done && (
              <button onClick={() => { setRows([]); setFileName(""); setDone(false); setSummary(null); setSelectedLocation(""); }}
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 rounded-2xl border border-[#4A6D55] bg-white text-[#4A6D55] text-sm font-bold hover:bg-green-50 transition-all">
...
export default function ManagePelanggan() {
  ...
  const [exportingLoc, setExportingLoc] = useState(false);
  ...
  const fetchLocations = async () => {
    ...
    if (!token) return;
    try {
      const res = await axios.get(`${API_BASE}/wilayah`, getAuthConfig());
      const raw: any = res.data;
      const list: Location[] = Array.isArray(raw)
        ? raw
        : Array.isArray(raw?.data)
        ? raw.data
        : Array.isArray(raw?.wilayah)
        ? raw.wilayah
        : [];
      setLocations(list.filter((l: Location) => l.isActive));
    } catch (error) {
    ...
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-6 space-y-4 md:space-y-6 text-black">
      ...
      <div className="bg-gradient-to-r from-[#DDE9E1] to-[#E8F1EB] rounded-[24px] p-5 sm:p-6 md:p-8 shadow-sm border border-white/50 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <span className="bg-white/60 text-[#4A6D55] px-3 md:px-4 py-1.5 rounded-full text-[10px] md:text-xs font-bold tracking-wider uppercase inline-block mb-2 md:mb-3">
            Data Pelanggan
          </span>
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-[#1A2E35] tracking-tight uppercase">Manajemen Pelanggan Retribusi</h1>
          <p className="text-sm md:text-base text-[#5B7078] mt-1 md:mt-2 font-medium">Kelola daftar pelanggan retribusi persampahan per wilayah</p>
        </div>
      </div>
        ...
        {[
          ...
          <div key={i} className="bg-white p-4 sm:p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col sm:flex-row items-center gap-3 sm:gap-4 hover:shadow-md transition-all">
            ...
            <div className="text-center sm:text-left">
              ...
              <p className="text-xl sm:text-2xl font-black text-gray-900 mt-0.5">{s.val}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-2xl px-4 py-2.5 shadow-sm flex-1 sm:flex-none">
          <MapPin size={16} className="text-gray-400" />
          <select
            value={filterLocation}
            onChange={(e) => setFilterLocation(e.target.value)}
            className="text-sm text-gray-700 outline-none bg-transparent font-medium w-full"
          >
        ...
        <button onClick={() => setShowImport(true)}
          className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-3 rounded-2xl border border-[#4A6D55] bg-white text-[#4A6D55] text-sm font-bold shadow-sm hover:bg-green-50 active:scale-95 transition-all">
          <FileSpreadsheet size={18} /> Import Excel
        </button>

        {filterLocation ? (
          <button onClick={handleExportByLocation} disabled={exportingLoc}
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-3 rounded-2xl border border-blue-500 bg-white text-blue-600 text-sm font-bold shadow-sm hover:bg-blue-50 active:scale-95 transition-all disabled:opacity-50">
            {exportingLoc ? <Loader2 size={16} className="animate-spin" /> : <Download size={18} />}
            Export Wilayah Ini
          </button>
        ) : (
          <button onClick={handleExportAll} disabled={exportingAll}
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-3 rounded-2xl border border-gray-300 bg-white text-gray-700 text-sm font-bold shadow-sm hover:bg-gray-50 active:scale-95 transition-all disabled:opacity-50">
              ...
              <thead>
                <tr className="bg-gray-50 text-gray-400 text-[10px] font-bold uppercase tracking-widest border-b border-gray-50">
                  {["No", "Nama Pelanggan", "Alamat", "Jenis Usaha", "Wilayah", "Aksi"].map((h) => (
                    <th key={h} className="px-4 sm:px-6 py-3 sm:py-4">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {list.length === 0 ? (
                  ...
                  <tr key={p.id} className="hover:bg-gray-50/50 transition-colors group">
                    <td className="px-4 sm:px-6 py-3 sm:py-5 text-center text-sm font-bold text-gray-400">
                      {(pagination.page - 1) * pagination.limit + idx + 1}
                    </td>
                    <td className="px-4 sm:px-6 py-3 sm:py-5">
                      <div className="flex items-center gap-3">
                        <div className="h-8 w-8 sm:h-10 sm:w-10 bg-gray-50 rounded-xl flex items-center justify-center text-gray-400 group-hover:bg-green-50 group-hover:text-[#4A6D55] border border-gray-100 transition-colors font-black text-xs sm:text-base">
                          {p.nama.charAt(0).toUpperCase()}
                        </div>
                        <p className="font-bold text-gray-900 text-sm sm:text-base">{p.nama}</p>
                      </div>
                    </td>
                    <td className="px-4 sm:px-6 py-3 sm:py-5">
                      <div className="flex items-center gap-2 text-sm text-gray-600">
                        <MapPin size={14} className="text-gray-400 flex-shrink-0" />
                        <span className="text-xs sm:text-sm">{p.alamat || <span className="text-gray-400">—</span>}</span>
                      </div>
                    </td>
                    <td className="px-4 sm:px-6 py-3 sm:py-5 text-sm text-gray-600">{p.jenisUsaha || "—"}</td>
                    <td className="px-4 sm:px-6 py-3 sm:py-5">
                      {p.location?.name ? (
                        <span className="inline-flex items-center gap-1.5 px-2 sm:px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-[10px] sm:text-xs font-bold">
                          <MapPin size={10} /> {p.location.name}
                        </span>
                      ) : (
                        <span className="inline-flex items-center px-2 sm:px-3 py-1 rounded-full bg-amber-50 text-amber-600 text-[10px] sm:text-xs font-bold">
                          Tanpa Wilayah
                        </span>
                      )}
                    </td>
                    <td className="px-4 sm:px-6 py-3 sm:py-5">
                      <div className="flex gap-1.5">
                        <button type="button" onClick={() => setViewingItem(p)}
                          className="p-1.5 sm:p-2 text-white bg-blue-500 rounded-lg hover:bg-blue-600 transition-all shadow-sm" title="Lihat Detail">
                          <Eye size={14} />
                        </button>
                        <button type="button" onClick={() => openEdit(p)}
                          className="p-1.5 sm:p-2 text-white bg-yellow-400 rounded-lg hover:bg-yellow-500 transition-all shadow-sm" title="Edit Data">
                          <Edit3 size={14} />
                        </button>
                        <button type="button"
                          onClick={() => { setSelectedPelangganForDelete(p); setShowDeleteConfirm(true); }}
                          className="p-1.5 sm:p-2 text-white bg-red-500 rounded-lg hover:bg-red-600 transition-all shadow-sm" title="Hapus">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </td>
                ...
                .map((p, i) =>
                  p === "..." ? (
                    ...
                    <button key={p} onClick={() => fetchList(p as number)}
                      className={`w-8 h-8 sm:w-9 sm:h-9 rounded-xl text-sm font-bold transition-all ${pagination.page === p ? "bg-[#4A6D55] text-white shadow-sm" : "border border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
                      {p}
                    </button>
                  )
                )}
      ...
      <AnimatePresence>
        {viewingItem && (
          <div className="fixed top-0 left-0 w-full h-full z-[9999] flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 15 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 15 }}
              className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden my-auto border border-gray-100 max-h-[90vh] flex flex-col"
            >
              <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/70">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="font-extrabold text-gray-900 tracking-tight text-base sm:text-lg">Detail Pelanggan</h3>
                </div>
              ...
              <div className="p-4 sm:p-6 space-y-4 sm:space-y-5 overflow-y-auto">
                <div className="flex flex-col items-center gap-3">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 bg-green-50 text-[#4A6D55] border border-green-100 rounded-full flex items-center justify-center text-2xl sm:text-3xl font-black">
                    {viewingItem.nama.charAt(0).toUpperCase()}
                  </div>
                  <h4 className="text-lg sm:text-xl font-extrabold text-gray-900">{viewingItem.nama}</h4>
                </div>
                <div className="bg-gray-50 p-4 sm:p-5 rounded-2xl border border-gray-100 space-y-3 sm:space-y-4">
                  {[
                    { label: "Alamat",      value: viewingItem.alamat || "—" },
                    { label: "Jenis Usaha", value: viewingItem.jenisUsaha || "—" },
                    { label: "Wilayah",     value: viewingItem.location?.name || "Belum ditetapkan" },
                  ].map(({ label, value }) => (
                    <div key={label}>
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{label}</label>
                      <p className="text-gray-800 font-bold mt-1 text-sm sm:text-base">{value}</p>
                    </div>
                  ))}
                </div>
                <button type="button" onClick={() => setViewingItem(null)}
                  className="w-full py-3.5 bg-black text-white rounded-xl font-bold hover:bg-gray-800 transition-all text-sm sm:text-base">
                  Tutup Detail
                </button>
              </div>
            </motion.div>
        ...
        {showModal && (
          <div className="fixed top-0 left-0 w-full h-full z-[9999] flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
            <motion.div
              ...
              className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden my-auto border border-gray-100 flex flex-col max-h-[90vh]"
            >
              <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/70">
              ...
              <form key={editingItem ? `edit-${editingItem.id}` : "create-pelanggan"} onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 overflow-y-auto" noValidate>
                ...
                <div className="flex flex-col space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" value={formData.nama} onChange={(e) => setField("nama", e.target.value)}
                      ...
                      className={`w-full pl-11 pr-4 py-3 bg-gray-50/80 rounded-xl outline-none text-sm sm:text-base font-medium transition-all border ${
                        formErrors.nama
                          ? "border-red-300 focus:border-red-400 focus:ring-4 focus:ring-red-500/10 bg-red-50/10"
                          : "border-gray-200 focus:border-[#4A6D55] focus:ring-4 focus:ring-[#4A6D55]/10 focus:bg-white"
                      }`} />
                ...
                <div className="flex flex-col space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <select value={formData.locationId} onChange={(e) => setField("locationId", e.target.value)}
                      className={`w-full pl-11 pr-10 py-3 bg-gray-50/80 rounded-xl outline-none text-sm sm:text-base font-medium transition-all border appearance-none ${
                ...
                <div className="pt-4 flex flex-col sm:flex-row items-center gap-3 w-full">
                  <button type="button" onClick={closeFormModal}
                    className="w-full sm:flex-1 py-3.5 bg-gray-100 hover:bg-gray-200 active:scale-[0.99] text-gray-600 rounded-xl font-bold transition-all text-sm sm:text-base text-center disabled:opacity-50">
                    Batal
                  </button>
                  <button type="submit" disabled={submitting}
                    className="w-full sm:flex-1 py-3.5 bg-[#4A6D55] hover:bg-[#3d5a46] active:scale-[0.99] text-white rounded-xl font-bold shadow-md shadow-green-900/10 transition-all disabled:opacity-50 disabled:pointer-events-none flex items-center justify-center gap-2 text-sm sm:text-base">
                    {submitting
