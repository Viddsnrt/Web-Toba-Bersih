...
export default function ManageSupir() {
  ...
  return (
    ...
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-6 space-y-4 md:space-y-6 text-black">
      ...
      {/* SEARCH AND FILTER */}
      <div className="bg-white rounded-2xl border-none shadow-sm p-3 md:p-4 flex flex-col sm:flex-row lg:flex-row gap-3 md:gap-4 justify-between items-stretch lg:items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            autoComplete="off"
            className="w-full pl-11 pr-4 py-3 bg-gray-50 border-none rounded-xl focus:ring-2 focus:ring-green-500/20 outline-none text-sm md:text-base transition-all"
          />
        </div>
        ...
        <div className="flex flex-wrap gap-1 md:gap-2 overflow-x-auto">
          {["ALL", "ACTIVE", "INACTIVE"].map((filter) => (
            <button
              ...
              className={`px-3 md:px-4 py-2 rounded-xl text-xs md:text-sm font-bold transition-all whitespace-nowrap ${
        ...
        <table className="w-full text-left border-spacing-0 min-w-[850px]">
          <thead>
            <tr className="bg-gray-50 text-gray-400 text-[10px] sm:text-xs font-bold uppercase tracking-widest">
              <th className="px-4 sm:px-6 py-3 sm:py-4 w-16">No</th>
              <th className="px-4 sm:px-6 py-3 sm:py-4">Profil Supir</th>
              <th className="px-4 sm:px-6 py-3 sm:py-4">Kontak Email / Telp</th>
              <th className="px-4 sm:px-6 py-3 sm:py-4">Status Akses</th>
            ...
            {loading ? (
              <tr>
                <td colSpan={5} className="px-4 sm:px-6 py-12 text-center">
                  <div className="flex flex-col items-center justify-center text-gray-400 gap-3">
                    ...
                    <span className="italic text-sm sm:text-base font-medium">Memuat data supir...</span>
                  </div>
                </td>
              </tr>
            ) : filteredSupir.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 sm:px-6 py-10 text-center text-gray-400 italic text-sm sm:text-base">
              ...
              filteredSupir.map((supir, index) => (
                <tr key={supir.id} className="hover:bg-gray-50/50 transition-colors group">
                  <td className="px-4 sm:px-6 py-3 sm:py-4 text-sm sm:text-base text-gray-400 font-bold">{index + 1}</td>
                  <td className="px-4 sm:px-6 py-3 sm:py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 sm:w-10 sm:h-10 bg-green-50 rounded-xl flex items-center justify-center border border-green-100 font-bold text-[#4A6D55] shrink-0 text-xs sm:text-base">
                        {supir.fullName.charAt(0).toUpperCase()}
                      </div>
                      <div className="flex flex-col gap-0.5">
                        <span className="text-sm sm:text-base font-bold text-gray-900">{supir.fullName}</span>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 sm:px-6 py-3 sm:py-4">
                    <div className="flex flex-col gap-1.5">
                      <div className="flex items-center gap-1.5 text-xs sm:text-sm text-blue-600 font-mono font-bold">
                        <Mail size={12} className="text-blue-400" />
                        {supir.email}
                      </div>
                      <div className="flex items-center gap-1.5 text-[10px] sm:text-xs font-mono text-gray-500 bg-gray-50 w-fit px-2 py-0.5 rounded border border-gray-100">
                  ...
                  <td className="px-4 sm:px-6 py-3 sm:py-4">
                    <span className={`inline-flex items-center gap-1.5 px-2 sm:px-3 py-1 rounded-full text-[10px] sm:text-xs font-bold ${
                  ...
                  <td className="px-4 sm:px-6 py-3 sm:py-4 text-right space-x-1 sm:space-x-2">
                    <button
                      ...
                      className="p-1.5 sm:p-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors inline-flex shadow-sm"
                      title="Lihat Detail"
                    >
                      <Eye size={14} />
                    </button>
                    <button
                      ...
                      className="p-1.5 sm:p-2 bg-yellow-400 text-white rounded-lg hover:bg-yellow-500 transition-colors inline-flex shadow-sm"
                      title="Edit Data"
                    >
                      <Edit3 size={14} />
                    </button>
                    <button
                      ...
                      className="p-1.5 sm:p-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors inline-flex shadow-sm"
                      title="Hapus Akun"
                    >
                      <Trash2 size={14} />
                    </button>
        ...
        {showModal && (
          <div className="fixed top-0 left-0 w-full h-full z-[9999] flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
            <motion.div
              ...
              className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden my-auto flex flex-col border border-gray-100 max-h-[90vh]"
            >
              ...
              <form 
                key={editingSupir ? `edit-${editingSupir.id}` : "create-supir"} 
        ...
        {viewingSupir && (
          <div className="fixed top-0 left-0 w-full h-full z-[9999] flex items-center justify-center p-3 sm:p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
            <motion.div
              ...
              className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden my-auto max-h-[90vh] flex flex-col"
            >
              ...
              <div className="p-4 sm:p-6 space-y-6 overflow-y-auto">
                <div className="flex flex-col items-center gap-3 pt-2">
                  <div className="w-16 h-16 sm:w-20 sm:h-20 bg-green-50 text-[#4A6D55] border border-green-100 rounded-full flex items-center justify-center text-2xl sm:text-3xl font-black shadow-sm">
                    {viewingSupir.fullName.charAt(0).toUpperCase()}
                  </div>
                  <div className="text-center">
                    <h4 className="text-lg sm:text-xl font-black text-gray-900">{viewingSupir.fullName}</h4>
                ...
                <div className="grid grid-cols-1 gap-3">
                  <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100 flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm sm:text-base font-bold text-gray-800 mt-0.5 break-all">{viewingSupir.email}</p>
                    </div>
                  </div>
                  <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100 flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm sm:text-base font-bold text-gray-800 mt-0.5">{viewingSupir.phoneNumber || "Belum ditambahkan"}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 sm:p-6 bg-gray-50 border-t flex justify-end">
                <button type="button" onClick={() => setViewingSupir(null)} className="w-full py-3.5 bg-black text-white rounded-xl font-bold hover:bg-gray-800 transition-all shadow-md text-sm sm:text-base">
                  Tutup Detail
                </button>
              </div>
            </motion.div>
