...
const isOverdue = (item: any): boolean => {
  ...
  if (["SELESAI", "BEKERJA", "LAPORAN_BARU"].includes(item.status))
    return false;
...
function usePenugasan() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const assigned = new Set(
        penugasanData.map((p: any) => p.report?.id).filter(Boolean),
      );
  ...
  const getEffectiveStatus = (item: any): string => {
    if (item.status === "DITUGASKAN" && isOverdue(item))
      return "TIDAK_DIKERJAKAN";
    return item.status;
  };
  ...
  const isTruckAvailable = (
    ...
    return !itemList
      ...
      .some(
        (i) =>
          Math.abs(sel - new Date(i.scheduledAt!).getTime()) <
          2 * 60 * 60 * 1000,
      );
  };
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    ...
    >
      <span
        className={`w-1.5 h-1.5 rounded-full mr-1.5 shrink-0 ${style.dot}`}
      />
      {label}
    </span>
  );
...
function TugaskanModal({
  ...
  const selectedDriverName = selectedTruck
    ? (getDriverFromTruck(selectedTruck)?.fullName ?? "Belum Ada Driver")
    : "";
        ...
        <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div>
            ...
            <p className="text-[11px] text-gray-500 mt-1 uppercase tracking-wider font-bold">
              {isEdit
                ? "Ubah armada, jadwal, atau lokasi"
            ...
            >
              ...
              {trukList.map((t: any) => (
                <option key={t.id} value={t.id}>
                  ...
                  {getDriverFromTruck(t)?.fullName ?? "Tanpa Driver"}
              ...
              {hasDriver ? (
                ...
                <>
                  ...
                  <p className="text-xs text-amber-800 mt-2 font-medium leading-relaxed">
                    Armada ini belum memiliki driver yang terhubung. Silakan
                    assign driver terlebih dahulu di menu Manajemen Armada.
                  </p>
                </>
              )}
            </motion.div>
          ...
          <div>
            ...
            {formErrors.scheduledAt ? (
              ...
              <p className="mt-2 text-[11px] text-gray-400 ml-1 font-medium">
                * Jam operasional:{" "}
                {OPERATIONAL_START.toString().padStart(2, "0")}:00 -{" "}
                {OPERATIONAL_END.toString().padStart(2, "0")}:00 WIB. Pastikan
                jadwal tidak bentrok dalam rentang 2 jam dengan penugasan lain.
              </p>
            )}
          </div>
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center p-4">
      ...
      >
            ...
            <div>
              ...
              <div className="bg-gray-50/50 rounded-2xl p-4 border border-gray-100">
                <p className="font-bold text-gray-900 text-base">
                  {laporan.user?.fullName || laporan.pelapor || "Anonim"}
                </p>
                ...
                {laporan.latitude && laporan.longitude && (
                  <div className="mt-3 inline-flex items-center gap-2 px-3 py-1.5 bg-white border border-gray-200 rounded-lg shadow-sm">
                    ...
                    <p className="text-xs text-gray-500 font-mono">
                      {formatCoordinate(laporan.latitude)},{" "}
                      {formatCoordinate(laporan.longitude)}
                    </p>
                  </div>
                )}
              </div>
        ...
        {/* Footer aksi akan disediakan oleh komponen induk, karena membutuhkan callback */}
        <div className="px-6 py-5 border-t border-gray-100 bg-white flex flex-wrap sm:flex-nowrap gap-3">
          ...
          {/* Tombol Tolak dan Tugaskan akan diberikan dari parent melalui props tambahan */}
        </div>
      </motion.div>
    </div>
  );
        ...
        <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              {name
                ? `Laporan "${name}" akan ditolak.`
                : "Laporan akan ditolak dan statusnya diubah."}
            </p>
          </div>
          <button
            onClick={onClose}
      ...
      <div className="flex items-center gap-1 px-2 border-x border-gray-100">
        {pageNumbers.map((page, i) =>
          page < 0 ? (
            <span
              key={`e-${i}`}
              className="px-2 text-gray-300 text-xs font-bold select-none"
            >
              …
            </span>
          ) : (
...
export default function ManagePenugasan() {
  ...
  const [deleteType, setDeleteType] = useState<"penugasan" | "laporan">(
    "penugasan",
  );
  ...
  const validateForm = (isEdit = false) => {
    ...
    if (!formData.truckId) {
      ...
      if (!t || !getDriverFromTruck(t))
        errors.truckId = "Armada harus memiliki driver.";
    }
    if (!formData.scheduledAt) {
      ...
      if (d < new Date()) {
        ...
        !isTruckAvailable(
          ...
          isEdit ? editingId || undefined : undefined,
        )
      ) {
    ...
    if (!formData.location.trim())
      errors.location = "Lokasi penugasan wajib diisi";
  ...
  const openTugaskan = (item: any, edit = false) => {
    ...
    setFormData({
      ...
      scheduledAt:
        edit && item.scheduledAt
          ? toDateTimeLocalValue(new Date(item.scheduledAt))
          : "",
      location: loc,
    });
    setIsEditMode(edit);
    ...
    try {
      ...
      if (isEditMode && editingId) {
        ...
        showAlert(
          ...
          "Laporan aduan telah ditugaskan ke armada.",
        );
      }
  ...
  const handleTolak = async () => {
    ...
    setSubmitting(true);
    try {
      await api.put(`/laporan/${tolakId}/tolak`, {
  ...
  const renderRowActions = (item: any) => {
    ...
    if (isLaporanBaru) {
      return (
        <>
          ...
          <button
            onClick={() => openTugaskan(item, false)}
          ...
          <button
            onClick={() => {
              setTolakId(item.id);
              setTolakName(item.location || "Laporan");
      ...
      return (
        <>
          ...
          <button
            onClick={() => {
              setDeleteId(item.id);
              setDeleteName(item.location || "Laporan");
              setDeleteType("laporan");
    ...
    return (
      <>
        ...
        {item.status !== "SELESAI" && (
          <button
            onClick={() => openTugaskan(item, true)}
        ...
        {(item.status === "DITUGASKAN" || item.status === "BEKERJA") && (
          <button
            onClick={() => {
              setDeleteId(item.id);
              setDeleteName(item.taskNumber || item.location || "Penugasan");
              setDeleteType("penugasan");
              setShowDeleteConfirm(true);
            }}
            className="p-2 bg-red-50 text-red-600 rounded-xl hover:bg-red-100 transition-colors"
          >
              ...
              {loading ? (
                <tr>
                  <td colSpan={6} className="text-center py-32 text-gray-400">
                    <Loader2
                      className="animate-spin text-[#4A6D55] mx-auto"
                      size={36}
                    />
                ...
                <tr>
                  <td
                    colSpan={6}
                    className="text-center py-32 text-gray-400 font-medium"
                  >
                    <ClipboardList
                      size={40}
                    ...
                    >
                      <td className="px-6 py-5 align-top">
                        <div className="flex items-start gap-3">
                          <div
                            className={`w-8 h-8 mt-0.5 rounded-full flex items-center justify-center shrink-0 ${overdue ? "bg-orange-100 text-orange-600" : "bg-gray-100 text-gray-500 group-hover:bg-[#DDE9E1] group-hover:text-[#4A6D55]"}`}
                          >
                            <User size={14} />
                          </div>
                        ...
                        {item.scheduledAt ? (
                          <div>
                            ...
                            <p className="text-xs font-bold text-gray-700 flex items-center gap-1.5">
                              ...
                              />
                              <span
                                className={overdue ? "text-orange-600" : ""}
                              >
          ...
          {loading ? (
            ...
            paginatedItems.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`p-5 ${overdue ? "bg-orange-50/30" : ""}`}
                >
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex items-center gap-3 min-w-0">
                      <div
                        className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${overdue ? "bg-orange-100 text-orange-600" : "bg-gray-100 text-gray-500"}`}
                      >
                        <User size={16} />
                      </div>
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-2 text-[11px] text-gray-500 mb-4 font-medium">
                    ...
                    {item.scheduledAt && (
                      <span
                        className={`flex items-center gap-1 px-2 py-1 rounded-md ${overdue ? "bg-orange-50 text-orange-600 font-bold" : "bg-gray-50 border border-gray-100"}`}
                      >
                        <Calendar size={12} />
                        {new Date(item.scheduledAt).toLocaleDateString(
                          "id-ID",
