    ...
    <div className="bg-white p-6 rounded-[25px] shadow-sm border border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 hover:shadow-md transition-all">
      ...
      <div className="flex-1">
        ...
        <h3 className="text-xl font-bold text-slate-800 mb-1">
          {item.lokasi || "Lokasi Pelaporan"}
        </h3>
        <p className="text-slate-500 text-sm mb-3">{item.description}</p>
        <button
          onClick={() =>
            bukaPeta(Number(item.latitude), Number(item.longitude))
          }
      ...
      <div className="flex gap-2 w-full md:w-auto">
        {item.status === "PENDING" && (
          <button
            onClick={() => onUpdate(item.id, "DITINDAKLANJUTI")}
            className="bg-blue-600 text-white px-6 py-3 rounded-2xl text-xs font-bold hover:bg-blue-700"
          >
            PROSES
          </button>
        )}
        {item.status !== "SELESAI" && (
          <button
            onClick={() => onUpdate(item.id, "SELESAI")}
            className="bg-green-600 text-white px-6 py-3 rounded-2xl text-xs font-bold hover:bg-green-700"
          >
            SELESAI
          </button>
        )}
        {showDelete && (
          <button
            ...
            className="bg-red-100 text-red-600 p-3 rounded-2xl hover:bg-red-600 hover:text-white transition-all"
          >
            <Trash2 size={20} />
          </button>
        )}
