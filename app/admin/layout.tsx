...
export default function AdminLayout({
  ...
  const getMenuIdFromPath = (path: string) => {
    ...
    if (path.includes("/admin/galleries") || path.includes("/admin/Galeri"))
      return "galleries";
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.log(
        "[ADMIN LAYOUT DEBUG] Stored role in localStorage:",
        storedRole,
      );
      ...
      if (role !== "ADMIN") {
        console.log(
          "[ADMIN LAYOUT] Redirecting to /unauthorized - role is",
          role,
        );
  ...
  return (
    <div className="flex min-h-screen bg-slate-50">
      ...
      <main className="flex-1 transition-all duration-300 min-w-0 ml-0 md:ml-[280px]">
        <div
          className={`w-full p-3 md:p-4 lg:p-6 xl:p-8 pt-16 md:pt-0 text-[16px] md:text-[18px] ${
            isMobile ? "px-3 py-4" : ""
          }`}
        >
          {children}
